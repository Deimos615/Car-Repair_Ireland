...
class ManagerQuoteController extends Controller
{
    ...
    public function index()
    {
        ...
        $up_quotes = DB::select('select tbl.*,us.approved_date,us.id user_idp, rn.make, rn.model, rn.version, rn.engine_cc, rn.year_of_manufacture, qr.quote_id, qr.price, qr.garage_id, qr.picked_date, qr.reply, qr.user_date, qr.deposit
            from users us
            left join(
        ...
        // $reply_quotes = DB::table('quote_replies')
        //         ->select('id', 'quote_id', 'garage_id', 'user_date')
        //         ->groupBy('quote_id')
        //         ->get();
        // $reply_quotes = DB::table('quote_replies')
        //         ->select('id', 'quote_id', 'garage_id', DB::raw('MAX(user_date) as user_date'))
        //         ->groupBy('quote_id')
        //         ->get();
        $reply_quotes = DB::table('quote_replies')
                ->select('quote_replies.id', 'quote_replies.quote_id', 'garage_id', 'user_date')
                ->leftJoin(DB::raw('(SELECT quote_id, MAX(user_date) as max_user_date FROM quote_replies WHERE user_date IS NOT NULL GROUP BY quote_id) as max_dates'), function ($join) {
                    $join->on('quote_replies.quote_id', '=', 'max_dates.quote_id')->on('quote_replies.user_date', '=', 'max_dates.max_user_date');
                })
                ->groupBy('quote_replies.quote_id')
                ->get();
        // dd($reply_quotes);
        ...
        $pre_quotes = DB::select('select tbl.*,us.approved_date,us.id user_idp, rn.make, rn.model, rn.version, rn.engine_cc, rn.year_of_manufacture, qr.quote_id, qr.price, qr.garage_id, qr.picked_date, qr.reply, qr.user_date, qr.deposit  
            from users us
            left join(
                select r.*, group_concat(s.detail separator ", ") detail from repairs r
                left join quotes q on q.repair_id=r.id
        ...
        $authReply = array();
        $otherReply = array();
        foreach ($pre_quotes as $key => $value) {
            $gid = $value->garage_id;
            $qid = $value->quote_id;
            if($key == 0) {
                $last_qid = $value->id;
