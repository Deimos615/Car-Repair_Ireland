...
class ManagerBookingController extends Controller
{
    ...
    public function index()
    {
        ...
        // Getting Previous Bookings
        $pre_bookings = DB::select('select tbl.*,us.approved_date,us.id user_idp, rn.make, rn.model, rn.version, rn.engine_cc, rn.year_of_manufacture, qr.quote_id, qr.price, qr.garage_id, qr.picked_date, qr.reply, qr.deposit  
            from users us
            left join(
                select r.*, group_concat(s.detail separator ", ") detail from repairs r
                left join quotes q on q.repair_id=r.id
        ...
        $authReply = array();
        $otherReply = array();
        foreach ($pre_bookings as $key => $value) {
            $gid = $value->garage_id;
            $qid = $value->quote_id;
            if($key == 0) {
                $last_qid = $value->id;
        ...
        $pre_bookings = $ret;
        $pastBooking = count($pre_bookings);
        // dd($upcomingBooking);

        return view('manager.managerBooking',[
            'up_bookings'=> $up_bookings,
